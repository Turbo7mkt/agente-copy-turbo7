...
| **Agente do Vercel (eve)** | Avaliado em ago/26 e descartado. Daria Slack, que é real, mas no plano Hobby só roda modelo free-tier (`gpt-5.6-luna-fast`) escrevendo copy que vai para lojista, e a calibragem de tom da base foi montada contra o Claude. Pior: um agente é instruções + ferramentas, então os passos 1 a 3 do protocolo voltariam a ser prompt em vez de código — o filtro determinístico de ângulos e o linter deixariam de ser garantia. Some ainda que os briefings, com análise de concorrência dos clientes, passariam por um modelo de terceiro. **Se um dia for retomado**, o desenho que preserva as travas é o agente consumir a API do painel (`/api/clientes/{slug}/angulos` e `/api/validar`) em vez de reimplementar a lógica |
